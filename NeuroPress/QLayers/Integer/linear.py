...
class FullQuant(BaseLinear):
    def __init__(self, in_features, out_features, bias=True, act_bits=8, weight_bits=8, weight_type="signed"):
        super(FullQuant, self).__init__(in_features, out_features, bias=bias)
        self.weight_bits = weight_bits
        self.act_bits = act_bits 
        self.weight_type = weight_type
        self.act_type = "signed"

    def forward(self, x):
        q_weights, scale_w, zero_point_w = forward_quantize_per_tensor(self.weight, bits=self.weight_bits, type=self.weight_type)
        q_x, scale_x, _ = forward_quantize_per_tensor(x, bits=self.act_bits, type=self.act_type)
...
class LinearW8A8(FullQuant):
    def __init__(self, in_features, out_features, bias=True):
        super(LinearW8A8, self).__init__(in_features, out_features, bias=bias, act_bits=8, weight_bits=8, weight_type="signed")


class LinearW4A8(FullQuant):
    def __init__(self, in_features, out_features, bias=True):
        super(LinearW4A8, self).__init__(in_features, out_features, bias=bias, act_bits=8, weight_bits=4, weight_type="unsigned")
