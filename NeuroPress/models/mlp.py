...
class MLP(Qmodel):
    ...
    Example:
        ...
        # Define the layer type (e.g., standard linear layers)
        layer_type = nn.Linear

        # Initialize the MLP with 3 layers
        model = MLP(
    ...
    def __init__(self, layer_type, input_size, hidden_size, num_classes, num_layers):
        ...
        Example:
            ```python
            # Continuing from the class example
            model = MLP(layer_type=nn.Linear, input_size=784, hidden_size=256, num_classes=10, num_layers=3)
            ```
        """
